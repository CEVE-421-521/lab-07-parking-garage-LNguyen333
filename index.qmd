...
subtitle: ""
week: 9
author: "Leanh Nguyen CEVE 421/521"
jupyter: julia-1.10
date: 2024-03-08
categories: [Lab]
...
function simulate_and_plot_npv(n_sows::Int, n_years::Int, demand_growth_rate::Float64, discount_rate::Float64, levels)
    # Generate an ensemble of SOWs
    sows = generate_sows(n_sows, n_years, demand_growth_rate, discount_rate)
    
    # Calculate NPV for each policy
    static_npvs = calculate_npv_for_policies(sows, levels, StaticPolicy)
    adaptive_npvs = calculate_npv_for_policies(sows, levels, AdaptivePolicy)
    
    # Calculate the average NPV for each number of levels
    average_static_npvs = calculate_average_npv(static_npvs)
    average_adaptive_npvs = calculate_average_npv(adaptive_npvs)
    
    # Plot the NPV as a function of the number of levels for each case
    plot(levels, average_static_npvs, label="Static Policy", marker=:circle)
    plot!(levels, average_adaptive_npvs, label="Adaptive Policy", marker=:square)
    xlabel!("Number of Levels")
    ylabel!("Average NPV")
    title!("Average NPV vs Number of Levels")
    display(plot)
end